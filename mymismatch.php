 ...
 function draw_bar_graph($width, $height, $data, $max_value, $filename){
        ...
        $bar_color = imagecolorallocate($img, 255, 10, 50);
 ...
 require_once('startsession.php');
  ...
  if(mysqli_num_rows($data) != 0){
  	...
  	$query = "SELECT mr.response_id, mr.topic_id, mr.response, mt.name AS topic_name, " . 
  				"mc.name AS category_name FROM mismatch_response AS mr " . 
  				"INNER JOIN mismatch_topic AS mt USING (topic_id) " .
  				"INNER JOIN mismatch_category AS mc USING (category_id) WHERE mr.user_id = '" . $_SESSION['user_id'] . "'";
  	$data = mysqli_query($dbc, $query);
    ...
    $mismatch_categories = array();
  	...
  	$data = mysqli_query($dbc, $query);
  	while($row = mysqli_fetch_array($data)){
  		//Grab the response data from the user (a potential mismatch)
  	...
  	if($mismatch_user_id != -1){
  		...
  		if(mysqli_num_rows($data) == 1){
  			...
  			echo '<table><tr class="login"><td>';

  			if(!empty($row['first_name']) && !empty($row['last_name'])){
  				echo $row['first_name'] . ' ' . $row['last_name'] . '</td></tr><tr class="login"><td>';
  			}
  				if(!empty($row['city']) && !empty($row['state'])){
  				echo $row['city'] . ', ' . $row['state'] . '';
  			}
  			echo '</td></tr><tr class="login"><td>';
  			if(!empty($row['picture'])){
  				echo '<img src="' . MM_UPLOADPATH . $row['picture'] . '" alt = "Profile Picture"><hr>';
  			}
  			echo '</td></tr></table>';

  			//Display the mismatched topics
  			echo '<h5 class="list">You are mismatched on the following ' . count($mismatch_topics) . ' topics:</h5><br><br>';
  			echo '<table border="10"><tr>';
        $i = 0;
        foreach ($mismatch_topics as $topic) {
          echo '<td class="login">' . $topic . '</td>';
          if (++$i > 3) {
            echo '</tr><tr>';
            $i = 0;
          }
      ...
      foreach($mismatch_categories as $category){
        ...
        echo '<h5>Mismatch Category Breakdown:</h5>';
        draw_bar_graph(480, 240, $category_totals, 5, MM_UPLOADPATH . $_SESSION['user_id'] . '-mymismatchgraph.png');
        echo '<img class="graph" src="' . MM_UPLOADPATH . $_SESSION['user_id'] . '-mymismatchgraph.png" alt="Mismatch Category Graph"><br>';
  			//Display a link to the mismatch user's profile
  			echo '<br><h4>View <a href="mismatchviewprof.php?user_id=' . $mismatch_user_id . '">' . 
  	...
  	echo '<p class="error">You must first <a href="mismatchquestion.php">answer the questionnaire</a> before you can' .
  	' be mismatched.</p>';
