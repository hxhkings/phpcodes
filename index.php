...
<body>
  ...
  <p>Welcome, have you had an encounter with extraterrestrials? Were you abducted? Have you seen my abducted dog, Moussey? <a href="reports.php">Report it here!</a></p>
...
<?php
  ...
  if (isset($_GET['abduction_id'])) {
    $query = "SELECT * FROM alien_abduction WHERE abduction_id = '" . $_GET['abduction_id'] . "'";
  }
  else {
    $query = "SELECT * FROM alien_abduction ORDER BY when_it_happened DESC LIMIT 6";
  }
  ...
  if (mysqli_num_rows($data) == 1) {
    ...
    echo '<strong>Moussey spotted:</strong> ' . $row['moussey_spotted'] . '</p>';
    ...
    while ($row = mysqli_fetch_array($data)) { 
      ...
      echo '<td><strong>Moussey spotted:</strong><br /> ' . $row['moussey_spotted'] . '</td></tr>';
    }
    echo '</table>';
